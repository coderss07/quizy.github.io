...
document.addEventListener("keydown", function(event){
    ...
    if (key == 123 || (event.ctrlKey && event.shiftKey && key == 73) || (event.ctrlKey && event.shiftKey && key == 74)) {
		document.getElementById('test').style.display = "none";
	    finishMsg(`You are not allowed to use dev tool!!`);
	}
}, false);
...
fetch("../quiz/assets/javascript/demo.json").then(response => response.json()).then(resp => {
	...
	document.getElementById('subject').innerHTML = resp.subject;
	offset = resp.time.offset;
	...
	if (n == 1) {
		...
		let option1 = resp.data[0].option1;
		let option2 = resp.data[0].option2;
		let option3 = resp.data[0].option3;
		let option4 = resp.data[0].option4;
		...
		let ele = `<div class="wrap" id="q${id}">
						...
						<div class="pt-4">
							<form class="form-d">
								<label class="options">${option1} <input  value="${option1}" type="radio" name="radio-${id}"> <span class="checkmark"></span> </label>
								<label class="options">${option2} <input  value="${option2}" type="radio" name="radio-${id}"> <span class="checkmark"></span> </label>
								<label class="options">${option3} <input  value="${option3}" type="radio" name="radio-${id}"> <span class="checkmark"></span> </label>
								<label class="options">${option4} <input  value="${option4}" type="radio" name="radio-${id}"> <span class="checkmark"></span> </label>
							</form>
						</div>
		...
		for (let i = 1; i < n - 1; i++) {
			...
			option1 = resp.data[i].option1;
			option2 = resp.data[i].option2;
			option3 = resp.data[i].option3;
			option4 = resp.data[i].option4;
		...
		option1 = resp.data[n - 1].option1;
		option2 = resp.data[n - 1].option2;
		option3 = resp.data[n - 1].option3;
		option4 = resp.data[n - 1].option4;
